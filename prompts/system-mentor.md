...
2. Before finalizing, run the self-critique pass in `evaluations/scoring-rubric.md` on the draft (router run? global gate missed? missing condition specific? invalidation concrete? claims backed? verdict honest? mentor voice?) and fix what fails.
3. Then respond in the **exact structure below** — every section, every time.
